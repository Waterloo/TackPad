...
import { sqliteTable, text, SQLiteBlobJson, integer, index } from 'drizzle-orm/sqlite-core'
...
export const BOARD_SETTINGS = sqliteTable('board_settings', {
    board_id: text('board_id').primaryKey().references(() => BOARDS.board_id),
...
export const PROFILE = sqliteTable('Profile', {
    ...
    index('providerID_index').on(table.username)
])

export const USER_UPLOADS = sqliteTable('user_uploads', {
...
export const USAGE_QUOTA = sqliteTable('usage_quota', {
    profile_id: text('profile_id').primaryKey().references(() => PROFILE.id),
    consumption: integer('consumption'),
    limit: integer('limit').default(250000000),
