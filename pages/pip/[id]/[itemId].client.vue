...
item.value = boardStore?.board?.data.items.find(item => item.id === route.params.itemId)
...
<template>
  <div v-if="isLoading" class="absolute inset-0 z-10 flex items-center justify-center bg-gray-100 bg-opacity-50">
    ...
    <TodoList v-else-if="item.kind === 'todo'" :list="item" :is-selected="boardStore.selectedId === item.id"
...
<style>
html, body,#__nuxt, .app-container > div {
  height: 100%;
}
</style>