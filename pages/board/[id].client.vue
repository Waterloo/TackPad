...
const compactSelectedItems = ref([null, null]);
onMounted(() => {
...
const compactItems = computed(() => {
    return boardStore.board.data.items;
});
...
<template>
    ...
    >
        ...
        <CompactWrapper
            v-if="boardStore.board?.data && compactMode == true"
            class="w-screen h-screen fixed left-0 bg-gray-100 z-20"
            :itemsList="boardStore.board.data.items"
            :selectedItemIds="compactSelectedItems"
            @updateSelection="updateList"
            :splitScreen="splitScreen"
            @toggleCompactMode="toggleCompactMode"
        >
            <template #screen1>
                <div v-for="item in compactItems">
                    <div
                        v-if="
                            compactSelectedItems.length > 0 &&
                            compactSelectedItems[0] == item.id
                        "
                    >
                        <StickyNote
                            v-if="item.kind === 'note'"
            ...
            <template #screen2>
                <div v-for="item in compactItems">
                    <div
                        v-if="
                            compactSelectedItems.length > 1 &&
                            compactSelectedItems[1] == item.id
                        "
                    >
                        <StickyNote
                            v-if="item.kind === 'note'"
    ...
    <BoardToolbar /> -->
        ...
        <div
            class="fixed top-20 right-5 bg-white p-2 rounded-full w-[36px] h-[36px]"
                ...
                >
                    ...
                    >
                        <path
                            ...
                            d="M8.92 8.714C8.495 7.39 9.476 6 10.867 6h26.266c1.391 0 2.372 1.39 1.947 2.714C37.9 12.4 36 19.09 36 24s1.9 11.6 3.08 15.286C39.505 40.61 38.524 42 37.133 42H10.867c-1.391 0-2.372-1.39-1.947-2.714C10.1 35.6 12 28.91 12 24S10.1 12.4 8.92 8.714M4 15c1.5 2 2 6 2 9s-.5 7-2 9"
                        />
