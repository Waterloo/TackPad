...
const filteredCommands = computed(() => {
  return commands.filter(command => 
    ...
    command.category.toLowerCase().includes(searchQuery.value.toLowerCase())
  );
});
...
watch(selectedIndex, async (newIndex) => {
  if (isOpen.value && commandListRef.value) {
    await nextTick();
    const selectedElement = commandListRef.value.querySelector(`[data-index="${newIndex}"]`);
    if (selectedElement) {
        ...
        >
          <div class="flex space-x-3">
            <div class="flex items-center space-x-1">
              <kbd class="px-1.5 py-0.5 bg-white border border-gray-300 rounded"
                >↑</kbd
              >
              <kbd class="px-1.5 py-0.5 bg-white border border-gray-300 rounded"
                >↓</kbd
