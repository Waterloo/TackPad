  ...
  <div class="w-full max-w-md mx-auto bg-white p-3 rounded-sm">
    ...
    <div class="bg-blue-500 rounded-full flex items-center p-1 pr-3 space-x-2">
      ...
      <div class="flex-grow relative">
        ...
        <div ref="waveformContainer" class="w-full h-8" :class="{ 'invisible': isLoading }"></div>
      </div>
  
      <!-- Time Display -->
      <span class="text-white text-sm w-20 text-right">
        {{ isLoading ? "--:--" : 
     (isPlaying || !isPlaying) ? `${formatTime(currentTime)} / ${formatTime(duration)}` : 
     formatTime(duration) }}
      </span>
    </div>
  </div>
...
onMounted(() => {
  if (waveformContainer.value) {
    ...
    // Add ready event to hide the loading indicator
