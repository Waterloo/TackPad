if(!window.myScriptConfig) {
    ...
    .then(board => {
      ...
      if (!board.data) {
        ...
        if (boardEncrypted) {
          ...
          const passwd = prompt("Please enter your board password:", "");
          if (passwd) {
            try {
              // Decrypt board using the available decrypt function
              const decryptedData = decrypt(board.data.content, passwd);
              ...
              const linkItem = {
                ...
                x_position: 0, // Default position instead of using findAvailablePosition
                y_position: 0, // Default position instead of using findAvailablePosition
                width: 300,
                height: 320
              };
              ...
              const encryptedData = encrypt(JSON.stringify(boardData), passwd);
  ...
  async function encrypt(data, passwd) {
    try {
      // No need to stringify data as it's already a string based on the usage
      const jsonString = typeof data === 'string' ? data : JSON.stringify(data);
      
      // Generate a random salt
      const salt = crypto.getRandomValues(new Uint8Array(16));
      
      // Derive key from passwd using PBKDF2
  ...
  async function decrypt(encryptedData, passwd) {
    try {
      ...
      // Derive the same key from passwd
      const keyMaterial = await crypto.subtle.importKey(
        'raw',
        str2ab(passwd),
        { name: 'PBKDF2' },
